"""Geothermal electricity ordinance content collection

These methods help filter down the document text to only the portions
relevant to utility-scale geothermal electricity generation ordinances.
"""
...
_LARGE_GEOTHERMAL_SYNONYMS = (
    "geothermal power plants, geothermal electricity generation facilities, "
    "geothermal energy facilities, utility-scale geothermal systems, "
    "commercial geothermal energy systems, geothermal generating facilities, "
    "geothermal production facilities, or similar"
)
_SEARCH_TERMS_AND = (
...
class GeothermalElectricityHeuristic(Heuristic):
    """Perform a heuristic check for geothermal power plants in text"""
    ...
    """Words/phrases: text is NOT about geothermal power plants"""
    GOOD_TECH_KEYWORDS = [
        "geothermal",
        "setback",
        "turbine",
    ...
    GOOD_TECH_ACRONYMS = []
    """Acronyms for geothermal power plants that we want to capture"""
    GOOD_TECH_PHRASES = [
        "turbine",
        "generator",
        "wellfield",
    ...
    """Phrases that indicate text is about geothermal power plants"""


class GeothermalElectricityOrdinanceTextCollector(StructuredLLMCaller):
    ...
    CONTAINS_ORD_PROMPT = (
        ...
        "keys. The first key is 'geothermal_reqs', which is a string that "
        f"summarizes all {_SEARCH_TERMS_AND} that are explicitly enacted "
        "in the legal text for geothermal electricity generation "
        "facilities for a given jurisdiction. "
        "Note that geothermal power plant bans are an important "
        "restriction to track. "
        "Include any **closely related provisions** if they clearly "
        "pertain to the **development, operation, modification, or "
        "removal** of geothermal electricity generation facilities (or "
        "geothermal power plants). "
        "All restrictions should be enforceable - ignore any text that "
        "only provides a legal definition of the regulation. If the text "
        f"does not specify any concrete {_SEARCH_TERMS_OR} for a "
        "geothermal electricity generation facility, set this key to "
        "`null`. The last key is '{key}', which is a boolean that is set "
        "to True if the text excerpt explicitly details "
        f"{_SEARCH_TERMS_OR} for a geothermal electricity generation "
        "facility and False otherwise."
    )
    """Prompt to check if chunk contains geothermal ordinance info"""

    IS_UTILITY_SCALE_PROMPT = (
        "You are a legal scholar that reads ordinance text and determines "
        f"whether it applies to {_SEARCH_TERMS_OR} for **utility-scale "
        "geothermal electricity generation facilities**. Utility-scale "
        "geothermal electricity generation facilities may "
        f"also be referred to as {_LARGE_GEOTHERMAL_SYNONYMS}. "
        "Your client is a geothermal power plant developer that does not "
        f"care about ordinances related to {_IGNORE_TYPES}. "
        "Ignore any text related to such systems. "
        ...
        "geothermal systems the text applies to (if any). The second key "
        "is '{key}', which is a boolean that is set to True if any part of "
        f"the text excerpt details {_SEARCH_TERMS_OR} for the **utility-scale "
        "geothermal electricity generation facilities** (or similar) that the "
        "client is interested in and False otherwise."
    )
    """Prompt to check if chunk is for utility-scale geothermal"""
...
class GeothermalElectricityPermittedUseDistrictsTextCollector(
    StructuredLLMCaller
):
    """Check text chunks for permitted geothermal districts"""

    DISTRICT_PROMPT = (
        "You are a legal scholar that reads ordinance text and determines "
        "whether it explicitly contains relevant information to determine the "
        "districts (and especially the district names) where "
        "utility-scale geothermal electricity generation facilities are a "
        "permitted use (primary, special, accessory, or otherwise), as "
        "well as the districts where geothermal power plants are "
        "prohibited entirely. Utility-scale "
        "geothermal electricity generation facilities may also "
        f"be referred to as {_LARGE_GEOTHERMAL_SYNONYMS}. "
...
class GeothermalElectricityOrdinanceTextExtractor(BaseTextExtractor):
    ...
    Responsibilities:
        1. Extract portions from chunked document text relevant to
           particular ordinance type (e.g. geothermal electricity
           generation zoning for utility-scale systems).
    Key Relationships:
        Uses a StructuredLLMCaller for LLM queries.
    """

    GEOTHERMAL_ENERGY_SYSTEM_FILTER_PROMPT = (
        "# CONTEXT #\n"
        "We want to reduce the provided excerpt to only contain information "
        "about **geothermal electricity generation facilities**. The "
        ...
        "applicable to geothermal electricity generation facilities** while "
        "preserving full context and accuracy.\n"
        "\n# OBJECTIVE #\n"
        "Extract all text **pertaining to geothermal electricity generation "
        "facilities** from the provided excerpt.\n"
        "\n# RESPONSE #\n"
        "Follow these guidelines carefully:\n"
        "\n1. ## Scope of Extraction ##:\n"
        "- Include **all** text that pertains to **geothermal "
        "electricity generation facilities**, even if they are referred "
        "to by different names such as:\n"
        f"\t{_LARGE_GEOTHERMAL_SYNONYMS.capitalize()}.\n"
        "- Explicitly include any text related to **bans or "
        "prohibitions** on geothermal electricity generation "
        "facilities.\n"
        "- Explicitly include any text related to the adoption or "
        "enactment date of the ordinance (if any).\n"
        "\n2. ## Exclusions ##:\n"
        "- Do **not** include text that does not pertain to geothermal "
    ...
    """Prompt to extract ordinance text for geothermal electricity"""

    async def extract_geothermal_energy_system_section(self, text_chunks):
        """Extract ordinance text from input text chunks for geothermal
        ...
        return await self._process(
            ...
            instructions=self.GEOTHERMAL_ENERGY_SYSTEM_FILTER_PROMPT,
    ...
    def parsers(self):
        ...
        yield (
            ...
            self.extract_geothermal_energy_system_section,
        )


class GeothermalElectricityPermittedUseDistrictsTextExtractor(
    BaseTextExtractor
):
    """Extract succinct ordinance text from input
